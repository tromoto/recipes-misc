...
static int get_kernel_version(void)
{
    ...
    fd = open(VERSION_FILE, O_RDONLY);    // Linux version 4.9 (oe-user@oe-host) (gcc
    if (fd < 0) {
        ALOGE("%s open failed\n", VERSION_FILE);
        return -1;
    }
    ...
    close(fd);
// no sublevel in /proc/version
//    ret = sscanf((const char *)version_str,  "Linux version %d.%d.%d", &version,  &patchlevel, &sublevel);
    ret = sscanf((const char *)version_str,  "Linux version %d.%d", &version,  &patchlevel);
///    if (ret != 3) {
//        ALOGE("sscanf error\n");
//        return -1;
//    }
    return KERNEL_VERSION(version, patchlevel, 119);
