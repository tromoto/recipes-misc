...
//  ge2d_config.src_format = GE2D_LITTLE_ENDIAN | GE2D_FORMAT_S32_ARGB ;
  ...
  switch (fb_vinfo.bits_per_pixel) {
    ...
    case 32:
      ...
      puts("ge2d_config.dst_format - 32");
      break;
    default:
      handle_error("Framebuffer has unsupported bits per pixel");
  }

  puts("1st ge2d_config call.");
  if (xioctl(ge2d_fd, GE2D_CONFIG, &ge2d_config) < 0)
    handle_error("GE2D_CONFIG Failed--");
//_IOW(GE2D_IOC_MAGIC, 0x00, &ge2d_config);
//GE2D_CONFIG;
  puts("1st ge2d_config call passed.");



  CLEAR(blit_rect);
  puts("CLEAR(blit_rect) done.");
...
//  if (xioctl(ge2d_fd, GE2D_BLIT_NOALPHA, &blit_rect) < 0)
    handle_error("GE2D blit Failed");

// maybe not needed if ALLOC_OSD0  ???
//  if (is_fbtft(fb_dev))
