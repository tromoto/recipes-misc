...
{
printf ("set_ge2dinfo GE2D_CANVAS_ALLOC = %d\n", GE2D_CANVAS_ALLOC)
...
int aml_generate_src1(char *src, aml_ge2d_info_t *pge2dinfo) // aml_generate_src1((char *)color_bar_code, &amlge2d.ge2dinfo);
{
    if (amlge2d.src_size[0] == 0)
        return 0;
    if (pge2dinfo->src_info[0].vaddr[0] == 0)
        return 0;
    printf("aml_generate_src1 check passed...\n");   // void *memcpy(void *dest, const void * src, size_t n)
	printf("aml_generate_src1 memcpy dest - %llX   src - %llX  size - %llX...\n", pge2dinfo->src_info[0].vaddr[0], &src, amlge2d.src_size[0]);
    memcpy(pge2dinfo->src_info[0].vaddr[0], src, amlge2d.src_size[0]);  // segmentation fault 
    printf("aml_generate_src1 memcpy passed...\n");
    return ge2d_success;
}
...
int main(int argc, char **argv)
{
    ...
    if (ret == ge2d_fail)
        return ge2d_success;
// 
    set_ge2dinfo(&amlge2d.ge2dinfo); // amlge2d is type aml_ge2d_t
    ...
    stime = myclock();
  printf("myclock passed...\n");

    for (i = 0; i < TEST_NUM; i++) {
        /* strechblit + rotate */
      printf("main aml_generate_src1 arg source %llX\n", &amlge2d.ge2dinfo );
        aml_generate_src1((char *)color_bar_code,&amlge2d.ge2dinfo);
      printf("main aml_generate_src1 dest %llx...\n", &amlge2d.ge2dinfo);
        amlge2d.ge2dinfo.ge2d_op = AML_GE2D_STRETCHBLIT;
        printf("main aml_generate_src1 passed...\n");
        ret = do_strechblit_rotate(&amlge2d.ge2dinfo);
        printf("do_strechblit_rotate passed...\n");
        if (ret < 0) {
