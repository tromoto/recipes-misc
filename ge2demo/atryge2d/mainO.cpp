...
// /dev/fb1 memory location from dmesg output
// mesonfb1(low)           : 0x07900000 - 0x07a00000 (1M)
//const unsigned long physicalAddress = 0xffffffc0021c3000;
//const unsigned long physicalAddress = 0x000000007a200000;
const unsigned long physicalAddress = 0x7a200000;
...
//    int fd = open("/dev/fb1", O_RDWR | O_SYNC);
    int fd = open("/dev/ion", O_RDWR | O_SYNC);
    if (fd < 0)
    {
        printf("Can't open /dev/ion (%x)\n", fd);
        printf("Can't open /dev/fb1 (%x)\n", fd);
        exit(1);
    }
    printf("Borrowed memory opened OK\n");
